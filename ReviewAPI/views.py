...
class FetchReviewsByIDView(APIView):
...
class FetchReviewsByLocationView(APIView):
    serializer_class = ReviewSerializer

    def get(self, request, name):

        try:
            reviews = Review.objects.filter(location__name__contains=name)
            serializer = FetchReviewSerializer(reviews, many=True)
            return Response(serializer.data, status.HTTP_200_OK)

        except Review.DoesNotExist:
            return Response("No reviews exist for this location", status.HTTP_404_NOT_FOUND)
